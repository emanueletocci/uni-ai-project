...
import it.unisa.diem.ai.torcs.controllers.Controller.Stage;
...
public class Client {
    ...
    private static boolean addestramento;
    private static boolean guidaAutonoma;

    /**
     * @param args is used to define all the options of the client. <port:N> is
     ...
     * server is running (default is localhost) <id:ClientID> is used to specify
     * the ID of the client sent to the server (default is championship2009)
     ...
     * is default value, that means unlimited number of steps) <stage:N> is used
     * to set the current stage: 0 is WARMUP, 1 is QUALIFYING, 2 is RACE, others
     * value means UNKNOWN (default is UNKNOWN) <trackName:name> is used to set
    ...
    public static void main(String[] args) {
        ...
        //SwingUtilities.invokeLater(() -> new CharReader((SimpleDriver) driver));
        driver.setStage(stage);
        driver.setTrackName(trackName);

        /* Build init string */
        double[] angles = driver.initAngles();
    ...
    private static void parseParameters(String[] args) {
        ...
        for (int i = 1; i < args.length; i++) {
            ...
            if (entity.equals("guidaAutonoma")) {
