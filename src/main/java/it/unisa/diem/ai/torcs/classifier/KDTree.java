...
class KDTree {
    ...
    public KDTree(List<Sample> points) {
        ...
        this.dimensions = FeatureType.values().length;
    ...
    public List<Sample> kNearestNeighbors(Sample target, int k) {
        PriorityQueue<Sample> pq = new PriorityQueue<>(k, Comparator.comparingDouble(target::euclideanDistance).reversed());
    ...
    private void kNearestNeighbors(KDNode node, Sample target, int k, int depth, PriorityQueue<Sample> pq) {
        ...
        double distance = target.euclideanDistance(node.point);
        if (pq.size() < k) {
            pq.offer(node.point);
        } else if (distance < target.euclideanDistance(pq.peek())) {
            pq.poll();
            pq.offer(node.point);
        }

        int axis = depth % dimensions;
        KDNode nearNode = (target.get(axis) < node.point.get(axis)) ? node.left : node.right;
        KDNode farNode = (nearNode == node.left) ? node.right : node.left;

        kNearestNeighbors(nearNode, target, k, depth + 1, pq);

        if (pq.size() < k || Math.abs(target.get(axis) - node.point.get(axis)) < target.euclideanDistance(pq.peek())) {
